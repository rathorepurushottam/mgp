...
import {View, StyleSheet, ScrollView, FlatList, RefreshControl, TouchableOpacity} from 'react-native';
...
const ScoreCard =({route}) => {
  ...
  let url = `${BASE_URL}leader-board?limit=10&skip=0&matchid=${route?.route?.params?.matchDetails?.MatchId}&contest_category_id=${route?.route?.params?.details?.contest_category_id}&user_id=${userData?._id}`;
  let urlTwo = `${BASE_URL}leader-board?limit=10&skip=0&matchid=${route?.route?.params?.matchDetails?.MatchId}&contest_category_id=${route?.route?.params?.details?.contest_category_id}&user_id=${userData?._id}`;
  useEffect(() => {
    if (
